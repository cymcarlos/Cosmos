...
{
    e820map_t *retemp;          // 返回结构体指针
    u32_t retemnr = 0;          //  数组长度
    mbsp->mb_ebdaphyadr = acpi_get_bios_ebda();
    mmap(&retemp, &retemnr);    //&retemp结构体指针的地址   获取数组和长度
    if (retemnr == 0)
    {
        kerror("no e820map\n");
    }
    if (chk_memsize(retemp, retemnr, 0x100000, 0x8000000) == NULL)  // 检查长度够不够
    {
...
{
    if (1 > move_krlimg(mbsp, (u64_t)(0x8f000), 0x1001))  // 检查这个栈的空间是否有东西
    {
...
{
    u64_t *p = (u64_t *)(KINITPAGE_PHYADR);   //顶级页目录   //KINITPAGE_PHYADR - 0x1000  
    //页目录指针地址
    u64_t *pdpte = (u64_t *)(KINITPAGE_PHYADR + 0x1000);    //KINITPAGE_PHYADR + 0x1000 - 0x2000, 这里现在是放了16 个地址， 预留了足够多的位置
    //页目录地址===>这个其实就是页表项了
    ...
    {
        p[mi] = 0;
        pdpte[mi] = 0;      // 虽然没512 这么长， 但是也没啥问题
    }
    // 页=====》 一共2m
    // 16 * 512 个页， 这里初始化的
    for (uint_t pdei = 0; pdei < 16; pdei++)
    {
        pdpte[pdei] = (u64_t)((u32_t)pde | KPDPTE_RW | KPDPTE_P);  // 16个页目录地址， 每个地址pde间隔0x1000(KPDPTE_RW | KPDPTE_P 没有计算前)

        for (uint_t pdeii = 0; pdeii < PGENTY_SIZE; pdeii++)       // 页表项里有512 个页， 每个页2M
        {   //大页KPDE_PS 2MB，可读写KPDE_RW，存在KPDE_P
            pde[pdeii] = 0 | adr | KPDE_PS | KPDE_RW | KPDE_P;
            adr += 0x200000;                                        //2MB =  2 * 2^20 * 8
        }
        pde = (u64_t *)((u32_t)pde + 0x1000);                       //  1 * 2^12 * 8 = 512 * 64 
    }
    // 虚拟地址开始地址的对应的顶级页目录的项（可能不是第0项）， 设置为第N项
    // 第N项指向的是 页目录指针的第0项
    // 顶级页目录第0 项也是指向 页目录指针的第0项
...
{
    // retemp 放e820map_t结构体指针的地址
    realadr_call_entry(RLINTNR(0), 0, 0);           // 这里已经算好， 且放到E80MAP_ADRADR地址上
    *retemnr = *((u32_t *)(E80MAP_NR));
    *retemp = (e820map_t *)(*((u32_t *)(E80MAP_ADRADR)));  //  retemp地址上 上的内容（指针）变了
    return;
}
...
e820map_t *chk_memsize(e820map_t *e8p, u32_t enr, u64_t sadr, u64_t size)
{
    ...
    for (u32_t i = 0; i < enr; i++)        // 这里是有一个满足就行了
    {
