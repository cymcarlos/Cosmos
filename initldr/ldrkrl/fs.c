...
{
    u64_t sz = r_file_to_padr(mbsp, IMGKRNL_PHYADR, "Cosmos.bin");      // Cosmos.bin内核文件放到IMGKRNL_PHYADR里
    if (0 == sz)
    {
    ...
    mbsp->mb_krlsz = sz;
    // mb_nextwtpadr4k 对齐 始终要保持指向下一段空闲内存的首地址
    mbsp->mb_nextwtpadr = P4K_ALIGN(mbsp->mb_krlimgpadr + mbsp->mb_krlsz); 
    mbsp->mb_kalldendpadr = mbsp->mb_krlimgpadr + mbsp->mb_krlsz;       // 实际上结束的地址
    return;
}
...
{
    u64_t sz = 0;
    u32_t dfadr = (u32_t)mbsp->mb_nextwtpadr;                   // 上一个4k对其的地址
    sz = r_file_to_padr(mbsp, dfadr, "font.fnt");               // 找font.fnt映像文件，且放置到dfadr里
    if (0 == sz)
    {
        kerror("r_file_to_padr err");
    }
...
void get_file_rpadrandsz(char_t *fname, machbstart_t *mbsp, u32_t *retadr, u32_t *retsz)
{
    ...
    padr = fhdsc->fhd_intsfsoff + mbsp->mb_imgpadr;     // 映像文件开始地址 + 文件偏移地址
    if (padr > 0xffffffff)
    {
        *retadr = 0;
        return;
    ...
    *retadr = (u32_t)padr;                               // 文件实际地址
    *retsz = (u32_t)fsz;                                 // 文件实际大小              
    return;
}
...
u64_t r_file_to_padr(machbstart_t *mbsp, u32_t f2adr, char_t *fnm)
{
    ...
    u32_t fpadr = 0, sz = 0;
    get_file_rpadrandsz(fnm, mbsp, &fpadr, &sz);  // 在映像中找fnm 文件名
    if (0 == fpadr || 0 == sz)
    {
        return 0;
    }
