...
#define REALDRV_PHYADR 0x1000                           // initldrsve.bin文件位置
#define ILDRKRL_PHYADR 0x200000                         // initldrkrl.bin文件位置
#define IMGSHEL_PHYADR 0x30000
#define IKSTACK_PHYADR (0x90000-0x10)                   //内核栈的开始地址
#define IKSTACK_SIZE 0x1000                             //内核栈的大小
#define IMGFILE_PHYADR 0x4000000
#define IMGKRNL_PHYADR   0x2000000                        // Cosmos.bin内核文件放的地址
#define KINITPAGE_PHYADR 0x1000000                      // 顶级页目录   16MB地址处
...
typedef struct s_e820{
    u64_t saddr;    /* start of memory segment8  开始地址*/
    u64_t lsize;    /* size of memory segment8   地址大小*/
    u32_t type;    /* type of memory segment     内存类型*/
...
typedef struct s_MACHBSTART                             // MBSPADR ((machbstart_t*)(0x100000))开始地址
{
    u64_t   mb_migc;          //LMOSMBSP//0
    u64_t   mb_chksum;//8
    u64_t   mb_krlinitstack;//16
    u64_t   mb_krlitstacksz;//24
    u64_t   mb_imgpadr;                                //映像文件开始地址指的是多个映像文件， 
    u64_t   mb_imgsz;                                  //映像文件大小 
    u64_t   mb_krlimgpadr;                             // 内核文件开始地址   IMGKRNL_PHYADR 0x2000000
    u64_t   mb_krlsz;                                  // 内核文件大小
    u64_t   mb_krlvec;
    u64_t   mb_krlrunmode;
    u64_t   mb_kalldendpadr;                           // 当前实际结束的地址，  没有4k对齐        区别于mb_nextwtpadr4k            
    u64_t   mb_ksepadrs;
    u64_t   mb_ksepadre;
    u64_t   mb_kservadrs;
    u64_t   mb_kservadre;
    u64_t   mb_nextwtpadr;                              // mb_nextwtpadr4k 对齐 始终要保持指向下一段空闲内存的首地址
    u64_t   mb_bfontpadr;                               // 字体文件开始地址
    u64_t   mb_bfontsz;                                 // 字体文件大小
    u64_t   mb_fvrmphyadr;
    u64_t   mb_fvrmsz;
    u64_t   mb_cpumode;
    u64_t   mb_memsz;                                   // 机器内存大小  ====>所有内存数组加起来
    u64_t   mb_e820padr;                                // 内存数组结构开始地址
    u64_t   mb_e820nr;                                  // 内存数组长度
    u64_t   mb_e820sz;                                  // 内存数组字节长度
    u64_t   mb_e820expadr;
    u64_t   mb_e820exnr;
    u64_t   mb_e820exsz;
    u64_t   mb_memznpadr;
    ...
    u64_t   mb_subpageslen;                             // 分页长度
    u64_t   mb_kpmapphymemsz;                           // 虚拟空间内存大小
    u64_t   mb_ebdaphyadr;
    mrsdp_t mb_mrsdp;
    graph_t mb_ghparm;                                  // 图形数据结构
}__attribute__((packed)) machbstart_t;

#define MBSPADR ((machbstart_t*)(0x100000))                     // 放的是二级引导收集的信息
