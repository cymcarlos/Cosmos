...
{
    machbstart_t *kmbsp = &kmachbsp;            // 不知道这个地址是啥
    machbstart_t *smbsp = MBSPADR;              // 二级引导整理的机器信息地址
    machbstart_t_init(kmbsp);                   // 清理0
    memcopy((void *)phyadr_to_viradr((adr_t)smbsp), (void *)kmbsp, sizeof(machbstart_t));  // 在长模式下，使用的是虚拟地址
    return;
}
