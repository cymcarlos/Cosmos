...
{
    gate_t *p_gate = &x64_idt[vector];                      // 中断向量表数组
    u64_t base = (u64_t)handler;                            // 处理程序地址
    p_gate->offset_low = base & 0xFFFF;                     // 低位偏移
    p_gate->selector = SELECTOR_KERNEL_CS;                  // 段选择子 todo  这里不太懂
    p_gate->dcount = 0;
    p_gate->attr = (u8_t)(desc_type | (privilege << 5));    // 属性
    p_gate->offset_high = (u16_t)((base >> 16) & 0xFFFF);   // 设置高位偏移
    p_gate->offset_high_h = (u32_t)((base >> 32) & 0xffffffff); // 高位的高位偏移
